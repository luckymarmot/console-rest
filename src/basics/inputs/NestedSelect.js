...
require('./nested-select.styl')

export default class NestedSelect extends Component {
    constructor(props) {
        super(props)
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({
            name: (nextProps.default || {}).name,
            value: (nextProps.default || {}).value,
            mode: (nextProps.default || {}).mode
        })
    }
    ...
    renderOptionsTitle() {
        if (this.props.title) {
            return <span className="select-options-title">
                {this.props.title}
            </span>
        }
